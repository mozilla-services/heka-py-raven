...
def transform(value, stack=[], context=None):
    ...
    if any(value is s for s in stack):
        ...
        try:
            ...
            # It's common case that a model's __unicode__ definition may try to
            # query the database which if it was not cleaned up correctly,
            # would hit a transaction aborted exception
...
def shorten(var, list_length=50, string_length=200):
    ...
    if isinstance(var, basestring) and len(var) > string_length:
        var = var[:string_length] + '...'
          ...
          and len(var) > list_length):
        # TODO: we should write a real API for storing some metadata with vars
        # when we get around to doing ref storage

        # TODO: when we finish the above, we should also implement this for
        # dicts
